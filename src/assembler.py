...
def asm_load(tokens):
    ...
    return bytes([0x39, r_dst, slot])

@register("MSTORE")
def asm_store(tokens):
    slot = parse_register(tokens[1])
    r_src = parse_register(tokens[2])
    return bytes([0x3a, slot, r_src])
...
def asm_mov(tokens):
    r_dst = parse_register(tokens[1])
    index = pad_bytes(parse_bytes_token(tokens[2]), 1)
    return bytes([0x50, r_dst]) + index

@register("RETURN")
def asm_mov(tokens):
    index = pad_bytes(parse_bytes_token(tokens[1]), 1)
    r_src = parse_register(tokens[2])
    return bytes([0x51]) + index + bytes([r_src])

@register("GETTXDATA")
def asm_mov(tokens):
    r_dst = parse_register(tokens[1])
    index = pad_bytes(parse_bytes_token(tokens[2]), 1)
    return bytes([0x52, r_dst]) + index

@register("TRANSFER")
def asm_transfer(tokens):
    r_address = parse_register(tokens[1])
    r_amount = parse_register(tokens[2])
    return bytes([0x53, r_address, r_amount])
